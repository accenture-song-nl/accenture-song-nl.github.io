myFT.on("ready", init);


function init() {

    document.querySelector("#mainExit").addEventListener("mouseover", function(){
        if(masterTL.progress() == 1){
            // gsap.to("#ctaRight", 0.3, {opacity:0.8, ease:Sine.easeInOut});
        }
    })
    
    document.querySelector("#mainExit").addEventListener("mouseout", function(){
        if(masterTL.progress() == 1){
            // gsap.to("#ctaRight", 0.3, {opacity:1, ease:Sine.easeInOut});
        }
    })

    document.querySelector("#mainExit").addEventListener("click", function(){
        masterTL.progress(1);
    })

    getAnimation();
}

function getAnimation(){

    masterTL = gsap.timeline({paused:true});
    masterTL.to("#loaderWrapper", 0.2, {opacity:0, ease:Sine.easeInOut}, 0)
    masterTL.add("intro")
    masterTL.from("#bg", 1, {scale:1.2, ease:Sine.easeInOut}, "intro");
    masterTL.from("#pups", 1, {scale:0.75, ease:Sine.easeInOut}, "intro");
    masterTL.from("#logoPP", 0.5, {y:-100, ease:Power2.easeOut}, "intro+=1");
    masterTL.from("#frame", 1, {opacity:0, x:-50, y:10, rotation:-20, ease:Power2.easeOut}, "intro+=1.2");
    
    masterTL.add("text1", "+=1")
    masterTL.to("#logoPP", 0.5, {y:-100, ease:Power2.easeIn}, "text1");
    masterTL.from("#text1", 0.5, {opacity:0, y:-20, ease:Power2.easeIn}, "text1+=0.5");
    masterTL.to("#bg", 1, {top: "-35px", ease:Sine.easeInOut}, "text1+=1");
    masterTL.to("#bluePanel", 1, {borderRadius:"15px 15px 0px 0px", height:143, ease:Sine.easeInOut}, "text1+=1");
    masterTL.from("#stickerDate", 1, {y:160, ease:Sine.easeInOut}, "text1+=1");
    masterTL.from("#sticker1", 1, {y:180, ease:Sine.easeInOut}, "text1+=1");
    masterTL.from("#sticker2", 1, {y:200, ease:Sine.easeInOut}, "text1+=1");
    masterTL.from("#sticker3", 1, {y:180, ease:Sine.easeInOut}, "text1+=1");
    masterTL.from("#stickerBadge", 1, {y:220, ease:Sine.easeInOut}, "text1+=1");
    masterTL.to(["#pups", "#frame"], 0.3, {opacity:0, ease:Power2.easeIn}, "text1+=1.7");
    masterTL.to("#sticker1", 1, {rotation:4, repeat:3, yoyo:true, ease:Sine.easeInOut}, "text1+=2");
    masterTL.to("#sticker2", 1, {rotation:-4, repeat:3, yoyo:true, ease:Sine.easeInOut}, "text1+=2");
    masterTL.to("#sticker3", 2, {rotation:4, repeat:1, yoyo:true, ease:Sine.easeInOut}, "text1+=2");
    
    masterTL.add("text2", "-=3")
    masterTL.to("#text1", 0.5, {opacity:0, ease:Power2.easeIn}, "text2");
    masterTL.from("#text2", 0.5, {opacity:0, y:-20, ease:Power2.easeOut}, "text2+=0.4");
    
    masterTL.add("showAlbum", "+=0")
    masterTL.to("#stickerDate", 1, {y:160, ease:Sine.easeInOut}, "showAlbum");
    masterTL.to("#sticker1", 1, {y:180, ease:Sine.easeInOut}, "showAlbum");
    masterTL.to("#sticker2", 1, {y:200, ease:Sine.easeInOut}, "showAlbum");
    masterTL.to("#sticker3", 1, {y:180, ease:Sine.easeInOut}, "showAlbum");
    masterTL.to("#stickerBadge", 1, {y:220, ease:Sine.easeInOut}, "showAlbum");
    masterTL.to("#text2", 0.5, {opacity:0, ease:Power2.easeIn}, "showAlbum");
    masterTL.from("#album", 1, {x:250, ease:Power2.easeOut}, "showAlbum+=0.8");
    masterTL.from("#albumText", 1, {x:280, ease:Power2.easeOut}, "showAlbum+=0.8");
    masterTL.from("#price", 1, {x:300, ease:Power2.easeOut}, "showAlbum+=0.8");
    masterTL.from("#cta1", 0.5, {scale:0, ease:Back.easeOut}, "showAlbum+=1.5");
    masterTL.to("#cta1", 0.5, {scale:1.1, repeat:1, yoyo:true, ease:Power2.easeInOut}, "showAlbum+=2.5");
    
    masterTL.add("endScreen", "+=0.5")
    masterTL.to("#album", 1, {x:-250, ease:Power2.easeOut}, "endScreen");
    masterTL.to("#albumText", 1, {x:-280, ease:Power2.easeOut}, "endScreen");
    masterTL.to("#price", 1, {x:-300, ease:Power2.easeOut}, "endScreen");
    masterTL.to("#cta1", 0.3, {scale:0, ease:Sine.easeIn}, "endScreen");
    
    masterTL.to("#bluePanel", 0.3, {height:153, ease:Sine.easeInOut}, "endScreen+=0.8");
    masterTL.to("#bg", 0.3, {top: "-45px", ease:Sine.easeInOut}, "endScreen+=0.8");
    masterTL.to("#text1", 0.5, {opacity:1, ease:Power2.easeIn}, "endScreen+=0.8");
    masterTL.from("#winImage", 1, {x:300, ease:Power2.easeOut}, "endScreen+=0.8");
    masterTL.from("#winText", 1, {x:280, ease:Power2.easeOut}, "endScreen+=0.8");
    masterTL.from("#cta2", 0.5, {scale:0, ease:Back.easeOut}, "endScreen+=1.5");
    masterTL.from("#highlight1Wrapper", 1, {width:0, ease:Power2.easeOut}, "endScreen+=1.8");
    masterTL.to("#cta2", 0.5, {scale:1.1, repeat:1, yoyo:true, ease:Power2.easeInOut}, "endScreen+=2.5");
    
    masterTL.play();

    console.log(masterTL.duration());
    
}