...
function init() {
    ...
    document.querySelector("#mainExit").addEventListener("mouseover", function(){
        if(masterTL.progress() == 1){
            gsap.to(".cta", 0.3, {scale:1.1, transformOrigin:"50% 650px", ease: "back.out(1.7)"})
        }
    })
...
function getAnimation(){
    ...
    masterTL.set(".whiteForeground", { right:0, left:'auto', ease: 'none'})
    masterTL.to(".whiteForeground", brandElementSpeed / 3, { width: "38%", ease: Power3.easeOut })
    
    masterTL.add("brandElementMovement", "start");
    masterTL.to(".squareBrandElement", brandElementSpeed, { y:-410, ease: Power2.easeInOut }, "brandElementMovement")
    ...
    masterTL.from([".bluePanel", ".addressInfo"], brandElementSpeed, { y:230, ease: Power3.easeInOut }, "brandElementAnim")
    masterTL.from(".bg", brandElementSpeed, { scale: 1.4, y: 0, ease: Power3.easeOut }, "brandElementAnim")
    masterTL.from([splitMainCopy.lines, '.cta', '.tagline'], brandElementSpeed, { y:45, opacity:0, ease: Power3.easeOut, stagger:0.15 }, "brandElementAnim+="+brandElementSpeed/2)
